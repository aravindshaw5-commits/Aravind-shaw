...
interface EditorialCardProps {
  project: Project;
  itemNumber: string; // e.g. "01" or "02"
...
export const EditorialCard: React.FC<EditorialCardProps> = ({
  ...
  return (
    ...
    >
      {/* ========================================================================= */}
      {/* 1. MEDIA AREA (1920 × 1080 px Landscape — Exact 16:9 Aspect Ratio) */}
      {/* ========================================================================= */}
      <div className="relative w-full aspect-video bg-slate-950 overflow-hidden select-none">
        {/* ITEM 1: STATIC IMAGE (No Video Element, No Play Button) */}
        {itemType === 'image' && (
          <div
            onClick={() => onSelect(project)}
            className="relative w-full h-full cursor-pointer group/thumb"
            title="Click to view book cover project details"
          >
            <img
              src={displayImage}
              alt={project.title}
            ...
            <div className="absolute top-3.5 left-3.5 flex items-center gap-2 pointer-events-none">
              <span className="px-2.5 py-1 rounded-md bg-white/95 backdrop-blur-xs text-emerald-800 text-[11px] font-mono font-bold shadow-xs border border-slate-200/80 flex items-center gap-1">
                ...
                <span>{itemNumber} • BOOK COVER</span>
              </span>
            </div>

            {/* Top Right: 16:9 Landscape Resolution Badge */}
            ...
            <div className="absolute bottom-3 left-3 pointer-events-none">
              <span className="px-2 py-1 rounded bg-black/60 backdrop-blur-xs text-white/90 text-[11px] font-medium border border-white/10">
                Front Cover Artwork
              </span>
            </div>
          </div>
        ...
        {/* ITEM 2: VIDEO SHOWCASE (Exact Reels Custom Play Button + Native Controls) */}
        {itemType === 'video' && (
          <>
            ...
            {/* Close Video / Return to Thumbnail Overlay Button */}
            ...
            {/* Thumbnail + EXACT REELS CUSTOM PLAY BUTTON (When Paused) */}
            {!isPlaying && (
              <div
                onClick={handleStartPlay}
                className="absolute inset-0 cursor-pointer group/thumb z-10 bg-slate-950"
                title="Click to play book opening video"
              >
                ...
                <div className="absolute top-3.5 left-3.5 flex items-center gap-2 pointer-events-none">
                  <span className="px-2.5 py-1 rounded-md bg-white/95 backdrop-blur-xs text-emerald-800 text-[11px] font-mono font-bold shadow-xs border border-slate-200/80 flex items-center gap-1">
                    ...
                    <span>{itemNumber} • BOOK OPENING VIDEO</span>
                  </span>
                </div>

                {/* Top Right: 16:9 Video Resolution Badge */}
                <div className="absolute top-3.5 right-3.5 pointer-events-none flex items-center gap-1.5">
                  <span className="px-2 py-0.5 rounded-md bg-slate-900/80 backdrop-blur-xs text-white text-[11px] font-mono font-medium border border-white/10 shadow-xs">
                    1920 × 1080
                  </span>
