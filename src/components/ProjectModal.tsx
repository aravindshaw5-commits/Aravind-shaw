          ...
          {!hasGallerySlots && (
            <div className={`relative rounded-xl overflow-hidden bg-slate-900 border border-slate-200 ${
              project.aspectRatio === 'portrait'
                ? 'aspect-[9/16] max-h-[520px] max-w-sm mx-auto'
              ...
              {project.videoUrl ? (
                ...
                <img
                  src={project.image}
                  alt={project.title}
                  className="w-full h-full object-contain md:object-cover object-center bg-slate-950"
                  referrerPolicy="no-referrer"
                />
              )}
              {project.highlight && (
                <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md text-slate-900 text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm flex items-center gap-1.5 pointer-events-none">
