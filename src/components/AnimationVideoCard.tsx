...
import { ArrowUpRight, Sparkles, X, AlertCircle, Clock, Calendar, Film, Play } from 'lucide-react';
...
export const AnimationVideoCard: React.FC<AnimationVideoCardProps> = ({
  project,
  onSelect,
  animNumber,
  featured = false
  ...
  // Safe Click-To-Play Handler: Directly triggers playback on the underlying video element
  ...
  // Close / Reset playback back to thumbnail view
  ...
  return (
    ...
    >
      {/* ========================================================================= */}
      {/* 1. VIDEO AREA (16:9 Aspect Ratio) */}
      {/* ========================================================================= */}
      <div className="relative w-full overflow-hidden bg-slate-950 aspect-video select-none">
        ...
        {/* Close to Thumbnail overlay button when video is playing */}
        ...
        {/* Center Play Button Overlay (Visible when paused or stopped) */}
        {!isPlaying && (
          <div
          ...
          >
            ...
            {/* Exact Reels Center Play Button (Matching Reels Component & Section 02) */}
            ...
            <div className="absolute top-3.5 left-3.5 flex items-center gap-2 pointer-events-none">
              {project.highlight ? (
                ...
                <span className="px-2.5 py-1 rounded-md bg-white/95 backdrop-blur-xs text-slate-700 text-[11px] font-semibold shadow-xs border border-slate-200/80">
                  {animNumber ? `${animNumber} • ${project.category}` : project.category}
                </span>
              )}
            </div>
      ...
      {/* ========================================================================= */}
      {/* 2 & 3. VIDEO TITLE / HEADLINE & DESCRIPTION */}
      {/* ========================================================================= */}
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        <div>
          {/* Header Row: Category / Project Tag */}
          <div className="mb-2 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-emerald-800 tracking-wider uppercase">
                {project.subtitle || project.category || '2D ANIMATION'}
              </span>
            </div>
          ...
          {/* Description Preview */}
          <p className="text-sm text-slate-600 line-clamp-2 sm:line-clamp-3 mt-2 leading-relaxed">
            {project.description}
          </p>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => onSelect(project)}
            className="shrink-0 text-xs font-semibold text-slate-700 hover:text-emerald-700 flex items-center gap-1 transition-colors cursor-pointer py-1 px-2 rounded-md hover:bg-slate-50"
          >
